...
<%@ page import="com.ta.dao.TAProfileDAO, com.ta.model.TAProfile, com.ta.util.SessionUtil, com.ta.util.CvMimeUtil" %>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<%@ include file="/jsp/common/header.jsp" %>
    ...
    <div class="card" style="max-width:640px; margin-top:20px;">
        ...
        <% if (profile!=null && profile.getCvFilePath()!=null && !profile.getCvFilePath().isEmpty()) {
            String ctx = pageContext.getRequest().getContextPath();
            boolean isPdf = CvMimeUtil.isPdf(profile.getCvFilePath());
        %>
            <div class="alert alert-success" style="margin-bottom:16px;">Current CV uploaded.</div>
            <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:12px;">
                <a href="<%= ctx %>/cv/serve?action=own&amp;disposition=inline" target="_blank" rel="noopener" class="btn btn-ghost btn-sm">Open in new tab</a>
                <a href="<%= ctx %>/cv/serve?action=own&amp;disposition=attachment" class="btn btn-primary btn-sm">Download</a>
            </div>
            <% if (isPdf) { %>
                <p class="text-sm text-muted mb-2">Preview</p>
                <iframe title="CV preview" style="width:100%;min-height:480px;border:1px solid var(--border-solid);border-radius:var(--radius);background:#fff;" src="<%= ctx %>/cv/serve?action=own&amp;disposition=inline"></iframe>
            <% } else { %>
                <p class="text-muted">In-browser preview is not available for this file type. Use <strong>Open in new tab</strong> or <strong>Download</strong>.</p>
            <% } %>
        <% } %>
